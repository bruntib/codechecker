...
def check(check_data):
    ...
    """
    actions_map, action, analyzer_config, ac, output_dirs, statistics_data = \
        check_data
    ...
    try:
        ...
        source_analyzer, rh = prepare_check(action, analyzer_config,
                                            ac.output_dir,
                                            ac.skip_handlers, statistics_data)
        ...
        def handle_analysis_result(success, zip_file=zip_file):
            ...
            if success:
                handle_success(rh, result_file, result_base,
                               ac.skip_handlers,
                               ac.capture_analysis_output,
                               success_dir)
            elif not ac.generate_reproducer:
                handle_failure(source_analyzer, rh,
                               os.path.join(failed_dir, zip_file),
                               result_base, actions_map, ac.skip_handlers)
        ...
        if rh.analyzer_returncode == 0:
            ...
            if ctu_active and ac.plugin_options['ctu_reanalyze_on_failure']:
...
def start_workers(ac: AnalysisConfig, actions_map, actions,
                  analyzer_config_map, metadata_tool, statistics_data, manager,
                  compile_cmd_count):
    """
        ...
        try:
            ...
            pool.map_async(check,
                           ...
                           callback=lambda results: worker_result_handler(
                               results, metadata_tool, ac.output_dir)
                           ).get(timeout)
